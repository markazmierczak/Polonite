...
void FileDescriptor::Close() {
  ...
  if (ret != 0)
    throw Exception::With(SystemException(GetLastPosixErrorCode()), "closing descriptor failed");
...
FileDescriptor FileDescriptor::Duplicate() {
  ...
  if (!rv.IsValid())
    throw Exception::With(SystemException(GetLastPosixErrorCode()), "failed to duplicate file descriptor");
  return rv;
}
...
void FileDescriptor::SetNonBlocking() {
  ...
  if (flags == -1)
    throw Exception::With(SystemException(GetLastPosixErrorCode()), "unable to get descriptor flags");
