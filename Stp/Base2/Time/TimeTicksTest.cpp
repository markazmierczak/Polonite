...
TEST(TimeTicksLogging, ZeroTime) {
  TimeTicks zero;
  EXPECT_EQ(StringSpan("0 bogo-microseconds"), FormattableToString(zero));
}

TEST(TimeTicksLogging, FortyYearsLater) {
  TimeTicks forty_years_later = TimeTicks() + TimeDelta::FromDays(365.25 * 40);
  EXPECT_EQ(StringSpan("1262304000000000 bogo-microseconds"), FormattableToString(forty_years_later));
}
