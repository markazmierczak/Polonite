...
Usage example:
  Tools/Linux/DumpStaticIntializers.py Out/Release/AppName
...
def main():
  ...
  parser.add_option('-t', '--toolchain-prefix', dest='toolchain',
                    ...
                    help='Toolchain prefix to append to all tool invocations (nm, objdump).')
  ...
  for filename, addr, size in files:
    ...
    if size == 2:
      ...
      for ref in ExtractSymbolReferences(opts.toolchain, binary, addr, addr+size):
        initializer_count += 1
    ...
    if opts.diffable:
      ...
      print '%s (initializer offset 0x%x size 0x%x)' % (qualified_filename, addr, size)
      print ''.join('  %s\n' % r for r in ref_output)

  if opts.diffable:
    print '#',
  print 'Found %d static initializers in %d files.' % (initializer_count, file_count)
