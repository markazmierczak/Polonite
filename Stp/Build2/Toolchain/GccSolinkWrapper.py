...
GccToolchain.gni's tool("solink"), in case the host running the compiler
...
def main():
  ...
  result = subprocess.call(
      WrapperUtils.CommandToRun(args.command), env=fast_env)
  if result != 0:
    return result
  ...
  if args.strip:
    result = subprocess.call(WrapperUtils.CommandToRun(
