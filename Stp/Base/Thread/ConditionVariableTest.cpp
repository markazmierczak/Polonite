...
class WorkQueue : public NativeThread::Delegate {
 public:
 ...
 private:
  ...
  List<NativeThreadObject> thread_handles_;
...
TEST_F(ConditionVariableTest, MAYBE_MultiThreadConsumerTest) {
  ...
  {
    AutoLock auto_lock(borrow(*queue.lock()));
  ...
  while (!queue.ThreadSafeCheckShutdown(kThreadCount))
    ThisThread::SleepFor(TimeDelta::FromMilliseconds(50));
}
...
TEST_F(ConditionVariableTest, LargeFastTaskTest) {
  ...
  AutoLock private_held_lock(borrow(private_lock));
  ConditionVariable private_cv(borrow(private_lock));
...
WorkQueue::WorkQueue(int thread_count)
  : lock_(),
    work_is_available_(borrow(lock_)),
    all_threads_have_ids_(borrow(lock_)),
    no_more_tasks_(borrow(lock_)),
  ...
  for (int i = 0; i < thread_count_; ++i) {
    ...
    EXPECT_NE(InvalidNativeThreadObject, pth);
...
WorkQueue::~WorkQueue() {
  ...
  for (int i = 0; i < thread_count_; ++i) {
    NativeThread::Join(thread_handles_[i]);
  }
  EXPECT_EQ(0, waiting_thread_count_);
}

int WorkQueue::GetThreadId() {
  ASSERT(!EveryIdWasAllocated());
...
void WorkQueue::SpinUntilAllThreadsAreWaiting() {
  while (true) {
    ...
    ThisThread::SleepFor(TimeDelta::FromMilliseconds(30));
  }
}
...
int WorkQueue::ThreadMain() {
  ...
  while (1) {  // This is the main consumer loop.
    ...
    if (work_time > TimeDelta::FromMilliseconds(0)) {
      ...
      AutoLock auto_lock(borrow(private_lock));
      ConditionVariable private_cv(borrow(private_lock));
