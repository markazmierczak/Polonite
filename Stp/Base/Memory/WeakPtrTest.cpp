...
class OffThreadObjectCreator {
 public:
  static T* NewObject() {
    T* result;
    {
      TaskThread creator_thread("creator_thread");
      creator_thread.Start();
      creator_thread.task_runner()->PostTask(
          FROM_HERE, Bind(OffThreadObjectCreator::CreateObject, &result));
    }
    ASSERT(result);  // We synchronized on thread destruction above.
    return result;
  }
 private:
...
class BackgroundThread : public TaskThread {
 public:
  BackgroundThread() : TaskThread("owner_thread") {}
  ...
  void CreateArrowFromTarget(Arrow** arrow, Target* target) {
    ...
    task_runner()->PostTask(
        FROM_HERE, Bind(&BackgroundThread::DoCreateArrowFromTarget, arrow,
                              target, &completion));
    completion.Wait();
  }
  ...
  void CreateArrowFromArrow(Arrow** arrow, const Arrow* other) {
    WaitableEvent completion(WaitableEvent::ResetPolicy::Manual,
                             WaitableEvent::InitialState::NotSignaled);
    task_runner()->PostTask(
        FROM_HERE, Bind(&BackgroundThread::DoCreateArrowFromArrow, arrow,
                              other, &completion));
    completion.Wait();
  }
  ...
  void DeleteTarget(Target* object) {
    ...
    task_runner()->PostTask(
        FROM_HERE,
        Bind(&BackgroundThread::DoDeleteTarget, object, &completion));
    completion.Wait();
  }
  ...
  void CopyAndAssignArrow(Arrow* object) {
    WaitableEvent completion(WaitableEvent::ResetPolicy::Manual,
                             WaitableEvent::InitialState::NotSignaled);
    task_runner()->PostTask(
        FROM_HERE, Bind(&BackgroundThread::DoCopyAndAssignArrow, object,
                              &completion));
    completion.Wait();
  }
  ...
  void CopyAndAssignArrowBase(Arrow* object) {
    WaitableEvent completion(WaitableEvent::ResetPolicy::Manual,
                             WaitableEvent::InitialState::NotSignaled);
    task_runner()->PostTask(
        FROM_HERE, Bind(&BackgroundThread::DoCopyAndAssignArrowBase,
                              object, &completion));
    completion.Wait();
  }
  ...
  void DeleteArrow(Arrow* object) {
    ...
    task_runner()->PostTask(
        FROM_HERE,
        Bind(&BackgroundThread::DoDeleteArrow, object, &completion));
    completion.Wait();
  }
  ...
  Target* DeRef(const Arrow* arrow) {
    ...
    task_runner()->PostTask(FROM_HERE, Bind(&BackgroundThread::DoDeRef,
...
TEST(WeakPtrDeathTest, WeakPtrCopyDoesNotChangeThreadBinding) {
  ...
  // WeakPtr.
  ASSERT_ASSERT_DEATH(background.DeRef(arrow_copy));
...
TEST(WeakPtrDeathTest, NonOwnerThreadDereferencesWeakPtrAfterReference) {
  ...
  BackgroundThread background;
  background.Start();
  ASSERT_ASSERT_DEATH(background.DeRef(&arrow));
}
