...
bool ReadProcMaps(std::string* proc_maps) {
  ...
  // TODO use FileStream
  ScopedFD fd(HANDLE_EINTR(open("/proc/self/maps", O_RDONLY)));
  if (!fd.IsValid()) {
    LOG(ERROR, "couldn't open /proc/self/maps");
    return false;
  }
  proc_maps->clear();
  ...
  while (true) {
    ...
    int bytes_read = HANDLE_EINTR(read(fd.get(), buffer, kReadSize));
    if (bytes_read < 0) {
      LOG(ERROR, "couldn't read /proc/self/maps");
      proc_maps->clear();
      return false;
    }
...
bool ParseProcMaps(const std::string& input,
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    if (sscanf(line, "%" SCNxPTR "-%" SCNxPTR " %4c %llx %hhx:%hhx %ld %n",
      ...
      LOG(ERROR, "sscanf failed for {} line", line);
      return false;
    }
